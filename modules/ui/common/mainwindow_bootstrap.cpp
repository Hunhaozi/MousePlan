...
{
    store.load();               // 从持久化存储加载数据
    store.ensureDefaultData();  // 确保必要的默认数据存在，例如默认用户、默认计划等
    store.save();               // 保存数据以确保任何默认数据的创建都被持久化
    selectedDate = QDate::currentDate();    // 初始化当前日期
}

// 功能：加载主题配置
void MainWindow::loadThemeConfig()
{
    const QString configPath = findConfigFilePath("theme.json");
    ...
    const QJsonObject colors = doc.object().value("colors").toObject();
    themePrimary = colors.value("primary").toString(themePrimary);
    themeAccent = colors.value("accent").toString(themeAccent);
    themeSoft = colors.value("soft").toString(themeSoft);
    themeTodayBg = colors.value("todayBg").toString(themeTodayBg);
    themeSelectedBg = colors.value("selectedBg").toString(themeSelectedBg);

    const QString uiConfigPath = findConfigFilePath("ui_config.json");
    if (uiConfigPath.isEmpty()) {
        return;
    }
    QFile uiFile(uiConfigPath);
    if (!uiFile.exists() || !uiFile.open(QIODevice::ReadOnly)) {
        return;
    }
    const QJsonDocument uiDoc = QJsonDocument::fromJson(uiFile.readAll());
    uiFile.close();
    if (!uiDoc.isObject()) {
        return;
    }

    const QJsonObject root = uiDoc.object();
    const QJsonObject dialogObj = root.value("dialog").toObject();
    gUiTuning.dialogBaseFont = dialogObj.value("baseFont").toInt(gUiTuning.dialogBaseFont);
    gUiTuning.dialogControlHeight = dialogObj.value("controlHeight").toInt(gUiTuning.dialogControlHeight);
    gUiTuning.dialogButtonHeight = dialogObj.value("buttonHeight").toInt(gUiTuning.dialogButtonHeight);
    gUiTuning.dialogCornerRadius = dialogObj.value("cornerRadius").toInt(gUiTuning.dialogCornerRadius);
    gUiTuning.dialogListItemHeight = dialogObj.value("listItemHeight").toInt(gUiTuning.dialogListItemHeight);

    const QJsonObject planManagerObj = root.value("planManager").toObject();
    gUiTuning.planManagerListFont = planManagerObj.value("listFont").toInt(gUiTuning.planManagerListFont);
    gUiTuning.planManagerActionFont = planManagerObj.value("actionFont").toInt(gUiTuning.planManagerActionFont);
    gUiTuning.planManagerActionHeight = planManagerObj.value("actionHeight").toInt(gUiTuning.planManagerActionHeight);
    gUiTuning.dialogTopRatio = planManagerObj.value("topRatio").toInt(gUiTuning.dialogTopRatio);
    gUiTuning.dialogContentRatio = planManagerObj.value("contentRatio").toInt(gUiTuning.dialogContentRatio);
    gUiTuning.dialogBottomRatio = planManagerObj.value("bottomRatio").toInt(gUiTuning.dialogBottomRatio);

    const QJsonObject loginObj = root.value("login").toObject();
    gUiTuning.loginLogoMinFont = loginObj.value("logoMinFont").toInt(gUiTuning.loginLogoMinFont);
    gUiTuning.loginCaptionMinFont = loginObj.value("captionMinFont").toInt(gUiTuning.loginCaptionMinFont);
    gUiTuning.loginInputMinFont = loginObj.value("inputMinFont").toInt(gUiTuning.loginInputMinFont);
    gUiTuning.loginButtonMinFont = loginObj.value("buttonMinFont").toInt(gUiTuning.loginButtonMinFont);
    gUiTuning.loginLinkMinFont = loginObj.value("linkMinFont").toInt(gUiTuning.loginLinkMinFont);
    gUiTuning.loginCardMinWidthPortrait = loginObj.value("cardMinWidthPortrait").toInt(gUiTuning.loginCardMinWidthPortrait);

    const QJsonObject mainObj = root.value("main").toObject();
    gUiTuning.mainBodyMinFont = mainObj.value("bodyMinFont").toInt(gUiTuning.mainBodyMinFont);
    gUiTuning.mainTopRatio = mainObj.value("topRatio").toInt(gUiTuning.mainTopRatio);
    gUiTuning.mainContentRatio = mainObj.value("contentRatio").toInt(gUiTuning.mainContentRatio);
    gUiTuning.mainBottomRatio = mainObj.value("bottomRatio").toInt(gUiTuning.mainBottomRatio);
    gUiTuning.calendarDayMinFont = mainObj.value("calendarDayMinFont").toInt(gUiTuning.calendarDayMinFont);
    gUiTuning.calendarNavMinFont = mainObj.value("calendarNavMinFont").toInt(gUiTuning.calendarNavMinFont);
    gUiTuning.navTextMinFont = mainObj.value("navTextMinFont").toInt(gUiTuning.navTextMinFont);
    gUiTuning.navIconMinSize = mainObj.value("navIconMinSize").toInt(gUiTuning.navIconMinSize);
    gUiTuning.mainActionButtonMinFont = mainObj.value("mainActionButtonMinFont").toInt(gUiTuning.mainActionButtonMinFont);
    gUiTuning.mainActionButtonMinHeight = mainObj.value("mainActionButtonMinHeight").toInt(gUiTuning.mainActionButtonMinHeight);
    gUiTuning.planItemTitleMinFont = mainObj.value("planItemTitleMinFont").toInt(gUiTuning.planItemTitleMinFont);
    gUiTuning.planItemMetaMinFont = mainObj.value("planItemMetaMinFont").toInt(gUiTuning.planItemMetaMinFont);
    gUiTuning.planItemButtonMinFont = mainObj.value("planItemButtonMinFont").toInt(gUiTuning.planItemButtonMinFont);
    gUiTuning.planItemMinHeight = mainObj.value("planItemMinHeight").toInt(gUiTuning.planItemMinHeight);

    const QJsonObject colorObj = root.value("colors").toObject();
    gUiTuning.dialogBg = colorObj.value("dialogBg").toString(gUiTuning.dialogBg);
}

// 功能：初始化根容器，方便后期不同界面之间的切换和布局管理
void MainWindow::setupRootUi()
{
    rootWidget = new QWidget(ui->centralwidget);            //根容器--空白界面
    rootOuterLayout = new QVBoxLayout(ui->centralwidget);   //创建一个垂直布局管理器
    rootOuterLayout->setContentsMargins(12, 12, 12, 12);    //上右下左的留白 12像素
    rootOuterLayout->addWidget(rootWidget);                 //添加根容器到垂直布局

    rootStackLayout = new QStackedLayout(rootWidget);       //创建一个堆叠布局，里面可以放入很多个界面，但是一次仅仅显示其中一个，其它的都隐藏
    rootStackLayout->setContentsMargins(0, 0, 0, 0);        //边上没有留白
}
...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), store(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation))
{
    // 全局拖放操作设置
    QApplication::setStartDragTime(4000);
    QApplication::setStartDragDistance(45);

    ui->setupUi(this);       // UI框架初始化
    initializeData();        // 数据加载与准备
    loadThemeConfig();       // 主题配置加载
    setupRootUi();           // 根UI结构搭建
    setupLoginPage();        // 登录界面初始化
    setupHomePage();         // 主页界面初始化
    applyThemeStyle();       // 应用主题样式
    applyResponsiveLayout(); // 响应式布局调整
    if (!tryAutoLoginOnStartup())
    { // 启动自动登录尝试
        switchToLoginPage();
    }
}
